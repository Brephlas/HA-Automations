...
LEDinfo(){
	...
	if [ "$option2" = "0" ]
		then
			curl -i -s -k -X $'POST' \
				    -H $'Host: 192.168.178.1' -H $'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:95.0) Gecko/20100101 Firefox/95.0' -H $'Accept: */*' -H $'Accept-Language: de,en-US;q=0.7,en;q=0.3' -H $'Accept-Encoding: gzip, deflate' -H $'Content-Type: application/x-www-form-urlencoded' -H $'Content-Length: 95' -H $'Origin: http://192.168.178.1' -H $'Connection: close' -H $'Referer: http://192.168.178.1/' \
				        --data-binary $"xhr=1&sid=$SID&led_change_state_event=14&menu_active_page=fn&apply=&lang=de&page=fn" \
					    $"http://$BoxIP/data.lua" &>/dev/null
	fi
	if [ "$option2" = "1" ]
		then
			curl -i -s -k -X $'POST' \
				    -H $'Host: 192.168.178.1' -H $'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:95.0) Gecko/20100101 Firefox/95.0' -H $'Accept: */*' -H $'Accept-Language: de,en-US;q=0.7,en;q=0.3' -H $'Accept-Encoding: gzip, deflate' -H $'Content-Type: application/x-www-form-urlencoded' -H $'Content-Length: 94' -H $'Origin: http://192.168.178.1' -H $'Connection: close' -H $'Referer: http://192.168.178.1/' \
				        --data-binary $"xhr=1&sid=$SID&led_change_state_event=5&menu_active_page=fn&apply=&lang=de&page=fn" \
					    $"http://$BoxIP/data.lua" &>/dev/null
	fi
	if [ "$option2" = "0" ]; then echo "INFO LED switched OFF"; fi
	if [ "$option2" = "1" ]; then echo "INFO LED switched ON"; fi
	# Logout
	wget -O - "http://$BoxIP/home/home.lua?sid=$SID&logout=1" &>/dev/null
}
